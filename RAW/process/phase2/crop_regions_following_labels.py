...
def is_valid_image(image):
    ...
    white_condition = white_percentage < 90
...
def random_proportional_crops(image_path, labels_path, image_filename, label_filename, output_dir, num_crops=10):
    ...
    for i, n in enumerate(arr):
        ...
        for idx in tqdm(indices, total=indices.shape[0]):
            ...
            cropped_labels = labels[y:y + crop_size, x:x + crop_size]
            crop_area = (x, y, crop_size, crop_size)
            if is_valid_image(cropped_labels):
                crops.append(crop_area)
                valid_crop_count += 1
                crop_index += 1
                cv2.imwrite(os.path.join(label_outdir, label_filename_no_ext + f"-crop-{crop_index}.png"), cropped_labels)
    del labels
    ...
    for id, crop in enumerate(crops):
        ...
        cv2.imwrite(os.path.join(image_outdir, image_filename_no_ext + f"-crop-{start_index + id}.png"), cropped_image)
        metadata.append({
