python infer_smooth.py \
    -opt options/uni_lora_cls.yml \
    --weight_path '/mnt/disk1/nmduong/Vin-Uni-Bone-Tumor/BoneTumor/src/exp/exp_cls_256_cls_rm_confused_case_with9/UNI_lora_cls/_best.pt' \
    --labels_dir '/mnt/disk1/nmduong/Vin-Uni-Bone-Tumor/RAW_DATA/labels' \
    --images_dir '/mnt/disk1/nmduong/Vin-Uni-Bone-Tumor/RAW_DATA/images' \
    --outdir './infer/smooth_old/'